...
namespace wiz{
	...
	template <class T>
	class BinomialHeap{ /// minHeap
	public:
		class BElement/// rename BElement to Element?
		{
		public:
			explicit BElement(const T& val){
				key = val;
	...
	public:
		static void Insert(BinomialHeap<T>** H, T key)
		{
			Insert(H, new BElement(key));
		}
		static T Extract(BinomialHeap<T>** H, const T& TINFINITY){
			BElement* temp = ExtractMinimum(H, TINFINITY);
			/// if temp == NULL then throw err?
			if (!temp) {
				throw wiz::Error(__FILE__, __LINE__, "Extract err");
			}
			T key = temp->key;
			delete temp;
			return key;
	...
	private:
		...
		//
	public:
		static void DecreaseKey(BElement* x, const T& k){
			if (NULL == x) {
				return;
			}
			if (x->key < k){
		...
		static bool Delete(BinomialHeap<T>** heap, BElement* x, const T& TINFINITY, const T& TMINUSINFINITY){
			BElement* temp;
			DecreaseKey(x, TMINUSINFINITY);
	...
	template < class T, class COMP = ASC<T>, class COMP2 = ASC<int>, class EE = EE<T> > /// default minHeap,
	class BinomialHeap2      /// COMP : ASC or PASC -> COMP2: ASC, COMP : DSC or PDSC -> COMP2 : DSC ??
	{
	private:
	...
	public:
		...
		{
			/// H1, H2 NULL test...
			BinomialHeap2<T, COMP, COMP2, EE>* temp = new BinomialHeap2<T, COMP, COMP2, EE>();
		...
		{
			class BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::BElement* temp = new class BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::BElement(WrapForInfinity<T, COMP, COMP2, EE>(key));
			BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::Insert(&heap, temp);
		}
		T Extract()
		{
			/// chk... COMP2 chk??
			if ((std::is_same< COMP, ASC<T> >::value)// T is not pointer type
				|| (std::is_same< COMP, PASC<T> >::value)) // T is pointer type.
				return BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::Extract(&heap, WrapForInfinity<T, COMP, COMP2, EE>::GetInfinity()).val;
			else if (std::is_same< COMP, DSC<T> >::value
					 || std::is_same< COMP, PDSC<T> >::value)
					 return BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::Extract(&heap, WrapForInfinity<T, COMP, COMP2, EE>::GetMinusInfinity()).val;
			else
				throw wiz::Error(__FILE__, __LINE__, " Extract err");
		}
		T GetMinimum()const
		{
			if (!std::is_same<COMP, ASC<T> >::value &&
				!std::is_same<COMP, PASC<T> >::value)
			{
				throw wiz::Error(__FILE__, __LINE__, " GetMinimum err ");
			}
			return BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::GetMinimum(heap, WrapForInfinity<T, COMP, COMP2, EE>::GetInfinity()).val;
		}
		T GetMaximum()const
		{
			if (!std::is_same<COMP, DSC<T> >::value &&
				!std::is_same<COMP, PDSC<T> >::value)
			{
				throw wiz::Error(__FILE__, __LINE__, " GetMaximum err ");
			}
			return BinomialHeap< WrapForInfinity<T, COMP, COMP2, EE> >::GetMinimum(heap, WrapForInfinity<T, COMP, COMP2, EE>::GetMinusInfinity()).val;
		}
		/// debug..
